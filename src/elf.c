...
static char *
elf_load_image (const char *path)
{
  ...
  if (ei_mag != ELF_MAGIC_NUMBER)
    {
      printf
	("Error loading program %s, unexpected magic number 0x%x - expected 0x%x\n",
...
static uint32_t
elf_get_brk (char *image, const char *path)
{
  ...
  for (i = 0; i < e_phnum; ++i)
	...
	{
	  printf
	    ("Error loading program %s, unexpected program header type 0x%x - expected 0x%x\n",
	...
	{
	  printf
	    ("Error loading program %s, it requested to be loaded at 0x%x, which is not allowed\n",
	     path, vadr);
	  return 0;
	}
    }
...
elf_load_process (char *image, const char *path)
{
  ...
  struct process *p =
    process_create (max_mem - PROCESS_ENTRY_POINT, PAGE_SIZE);
