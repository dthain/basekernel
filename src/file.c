...
static struct memory_inode_t *in_memory_fs;
static char *in_memory_fs_data;
static int memory_inode_count;

int file_init()
{
	in_memory_fs = kmalloc(IN_MEMORY_INODE_COUNT * sizeof(struct memory_inode_t));
...
{
	struct memory_inode_t *new_node = in_memory_fs + memory_inode_count;
