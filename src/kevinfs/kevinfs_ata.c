...
struct kevinfs_superblock *kevinfs_ata_read_superblock(int unit)
{
	struct kevinfs_superblock *check = kmalloc(sizeof(struct kevinfs_superblock));
	uint8_t buffer[FS_BLOCKSIZE];
	if (kevinfs_ata_read_block(unit, 0, buffer) < 0)
		return 0;
...
static int kevinfs_ata_write_superblock(int unit)
{
	...
	if (!ata_probe(unit, &num_blocks, &ata_blocksize, 0) || num_blocks == 0)
		return -1;
...
int kevinfs_ata_format(int unit)
{
	return kevinfs_ata_write_superblock(unit);
}


static int kevinfs_ata_get_available_bit(int unit, uint32_t index, uint32_t *res)
{
	uint32_t bit_index;
	uint8_t bit_buffer[FS_BLOCKSIZE];
	if (kevinfs_ata_read_block(unit, index, bit_buffer) < 0)
		return -1;
...
int kevinfs_ata_set_bit(int unit, uint32_t index, uint32_t begin, uint32_t end)
{
	...
	if (kevinfs_ata_read_block(unit, begin + bit_block_index, bit_buffer) < 0)
		return -1;
	if ((bit_mask & bit_buffer[bit_block_offset / 8]) > 0)
		return -1;

	bit_buffer[bit_block_offset / 8] |= bit_mask;
	return kevinfs_ata_write_block(unit, begin + bit_block_index, bit_buffer);
}

int kevinfs_ata_unset_bit(int unit, uint32_t index, uint32_t begin, uint32_t end)
{
	uint8_t bit_buffer[FS_BLOCKSIZE];
	uint32_t bit_block_index = index / (8 * FS_BLOCKSIZE);
	uint32_t bit_block_offset = index % (8 * FS_BLOCKSIZE);
...
int kevinfs_ata_check_bit(int unit, uint32_t index, uint32_t begin, uint32_t end, bool *res)
{
	uint8_t bit_buffer[FS_BLOCKSIZE];
	uint32_t bit_block_index = index / (8 * FS_BLOCKSIZE);
	uint32_t bit_block_offset = index % (8 * FS_BLOCKSIZE);
...
int kevinfs_ata_ffs_range(int unit, uint32_t start, uint32_t end, uint32_t *res)
{
	uint32_t index;
	int result;

	for (index = start; index < end; index++) {
		result = kevinfs_ata_get_available_bit(unit, index, res);
		if (result == 0) {
