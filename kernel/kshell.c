...
static int process_command(char *line)
{
	...
	if(pch && !strcmp(pch, "echo")) {
		...
		pch = strtok(0, " ");
		if(pch)
			fs_dirent_mkdir(current->current_dir, pch);
		else
		...
		pch = strtok(0, " ");
		if(pch)
			fs_dirent_rmdir(current->current_dir, pch);
		else
			printf("rmdir: missing argument\n");
	} else if(pch && !strcmp(pch, "chdir")) {
		pch = strtok(0, " ");
		if(pch)
			current->current_dir = fs_dirent_namei(current->current_dir, pch);
		else
		...
		printf("%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n", "Commands:", "echo <text>", "run <path>", "start <path>", "kill <pid>", "reap <pid>", "wait", "test <function>", "list", "time", "help", "exit", "stress",
		       "mount <unit_no> <fs_type>", "format <unit_no> <fs_type>", "mkdir <dir>", "chdir <dir>", "rmdir <dir>");
